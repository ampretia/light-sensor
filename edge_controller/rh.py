                ...
                if self.bits >= 12:  # Complete token.
                    ...
                    if self.byte >= self.message_length:
                        ...
                        if crc == 0xF0B8:  # Valid CRC.
                            ...
                            if (
                                headerTo == self.this_address
                                or headerFrom != self.this_address
                                and headerTo == RH_BROADCAST_ADDRESS
                            ):
...
if __name__ == "__main__":
    ...
    client.loop_start()
    client.connect("192.168.1.150", 1885, 60)

    ltrGainFactor = 1
    ltrResolutionFactor = 1
    asGain = 1
    asGainFactor = 1
    ltrGain = 1
    ltrResoultion = 1
    ...
    while running:
        while rx.ready():
            remote_msg = rx.get()
            data = "".join(chr(c) for c in remote_msg["msg"])
            logging.info("radiodata:: %s [%d]" % (data, remote_msg["headerMsgId"]))

            msgType = remote_msg["headerMsgId"]
            ...
            if msgType == 5:
                ltrGain = float(visual["ltrr"])
                ltrGainFactor = ltrGain / 18
                ltrResoultion = float(visual["ltrr"])
                ltrResolutionFactor = (2**ltrResoultion) / (2**20)

                asGain = int(visual["spg"])
                if asGain == 0:
                    asGainFactor = 2
                else:
                    asGainFactor = 1
                logging.info(
                    "Cfg:  asGain=%d  ltrGainFactory=%f ltrResoultionFactor=%f"
                    % (asGain, ltrGainFactor, ltrResolutionFactor)
                )

            if msgType == 3:
                uvi = float(visual["uv"]) / (
                    ltrGainFactor * ltrResolutionFactor * 2300.0
                )
                logging.info("Calulcate uvi of %f" % uvi)
                visual["uvi"] = "{:.2f}".format(uvi)

                lux = (float(visual["als"]) * 0.6) / (ltrGain * 0.25)
                logging.info("Calulcate lux of %f" % lux)
                visual["lux"] = "{:.2f}".format(lux)

            if msgType == 1:
                for x in range(1, 9):
                    visual[str(x)] = str(int(visual[str(x)]) * asGainFactor)
...
# 1:1009 2:2687 3:3865 4:4871 5:6476 6:907 7:10394 8:1898
